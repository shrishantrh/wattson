...
| API | https://wattson-api-ivory-seastar-408.fly.dev/ |

Both are live. The ask layer, Elastic corpus search and the Grok narration all answer from
the deployed API. The site also runs with no network at all: every endpoint is baked into
`web/dist/api/`, so a local build is the fallback if the venue wifi dies.
