...
export default function DetectorView({ detection: d }) {
  ...
  return (
    <Mod
      ...
      lead={<Lead value={rank != null ? `#${fmt(rank)}` : '—'} label={`of ${fmt(n)} regions scored on demand alone${score != null ? ` · score ${fmt(score, 2).replace(/^-/, '−')}` : ''}`} />}
      foot="Demand-only score, frozen before the ranking was seen. Bars are scaled ±20 pts, ±40 pts, ±0.10, ±60%; the figures are exact."
    >
