...
// The frozen demand-only detector. The card leads with the rank and the score beside it, then the
// four components as labelled tick bars, each in plain words, with its sign and its units.
      ...
      <div className="mod-ticks">
        {rows.map(r => (
          <div key={r.key} className="mod-tickrow">
            ...
            <span className="mod-tk"><Ticks value={r.v} max={r.max} n={10} accent={r.accent} label={`${r.t}${r.d ? `, ${r.d}` : ''}: ${r.n}`} /></span>
