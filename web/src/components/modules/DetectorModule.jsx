...
export default function DetectorView({ detection: d }) {
  ...
  const rows = [
    ex != null && { key: 'excess', t: `Night demand grew ${ex >= 0 ? 'faster' : 'slower'} than average demand`, d: 'overnight excess, percentage points', n: `${sgn(ex, 1)} pts`, v: Math.abs(ex), max: SCALE.excess, tone: ex >= 0 ? 'fossil' : 'neutral' },
    nd != null && { key: 'neighbours', t: `Grew ${nd >= 0 ? 'faster' : 'slower'} than its neighbours`, d: 'neighbour divergence, percentage points', n: `${sgn(nd, 1)} pts`, v: Math.abs(nd), max: SCALE.neighbours, tone: nd >= 0 ? 'fossil' : 'neutral' },
    lf != null && { key: 'lf', t: `Load got ${lf >= 0 ? 'flatter' : 'peakier'}`, d: 'load factor change, 2019 to 2025', n: sgn(lf, 3), v: Math.abs(lf), max: SCALE.lf, tone: lf >= 0 ? 'fossil' : 'neutral' },
    g != null && { key: 'growth', t: 'Demand growth since 2019', d: 'average MW, all hours', n: `${sgn(g, 1)}%`, v: Math.abs(g), max: SCALE.growth, tone: 'neutral' },
  ].filter(Boolean)
  return (
    <Mod
      className="mod-detector"
      caption={d.pattern ? `Pattern here: ${d.pattern} — a descriptive label, never part of the score.` : 'A demand-only score: no generation figure goes into it.'}
      ...
      <div className="mod-ticks">
        {rows.map(r => (
          <div key={r.key} className="mod-tickrow">
            ...
            <span className={`mod-tk ${r.tone}`}><Ticks value={r.v} max={r.max} n={12} /></span>
