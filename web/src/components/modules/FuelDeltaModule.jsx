...
// Fossil first, then carbon-free, then the neutral remainder. Opacity steps keep gas the loudest bar.
const ORDER = ['gas', 'coal', 'oil', 'nuclear', 'hydro', 'wind', 'solar', 'geothermal', 'other']
const FOSSIL = { gas: 0.9, coal: 0.6, oil: 0.45 }
...
// Change in average overnight generation by fuel, 2019 to 2025, in GW. Pure CSS bars on one shared
// scale: the zero line sits where the most negative bar ends, so every length is proportional to |value|.
export default function FuelDeltaView({ delta, grid, inherited }) {
  ...
  if (!rows.length) return <p className="mod-empty">No fuel breakdown for this grid.</p>
  ...
  return (
    <div className="mod-fuel">
      <div className="mod-sums">
        <div className="mod-sum"><span className="mod-sum-v accent">{fmtGw(fossil)}</span><span className="mod-sum-l">fossil: gas, coal, oil</span></div>
        <div className="mod-sum"><span className="mod-sum-v">{fmtGw(clean)}</span><span className="mod-sum-l">carbon-free: nuclear, hydro, wind, solar, geothermal</span></div>
      </div>
      <ul className="mod-bars">
        {rows.map(({ fuel, v }) => {
          const kind = fuel in FOSSIL ? 'fossil' : CLEAN.has(fuel) ? 'clean' : 'other'
          ...
          return (
            <li key={fuel} className={`mod-bar ${kind}`}>
              ...
              <span className={`mod-bar-val${fuel === 'gas' ? ' accent' : ''}`}>{fmtGw(v)}</span>
            </li>
          )
        })}
      </ul>
      <p className="mod-foot">Change in average overnight generation, 2019 to 2025, for the whole {inherited && grid ? `${grid} ` : ''}grid.</p>
    </div>
  )
}
