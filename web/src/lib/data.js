...
// Live API origin. In DEV we always use same-origin `/api` (Vite proxies to :8000) so
// the browser never cross-origin-fetches localhost:8000 (avoids Failed to fetch / CORS flakes).
// Outside DEV, VITE_API_BASE is required for narrate/script.
const api = (import.meta.env.VITE_API_BASE || '').replace(/\/$/, '')
const liveRoot = () => (import.meta.env.DEV ? '' : (api || null))
const liveUrl = path => {
  const root = liveRoot()
...
// ---------- irradiance overlay (day vs night vs sunlight) ----------
export async function loadIrradiance() {
  const live = liveUrl('/api/irradiance')
  return tryEach([
    ...(live != null ? [() => getJSON(live)] : []),
    ...(api && !import.meta.env.DEV ? [() => getJSON(`${api}/api/irradiance`)] : []),
    () => staticExport('irradiance'),
    () => fixture('irradiance'),
  ])
}

/** POST /api/irradiance/narrate → MP3 blob. Uses Vite proxy in dev (same-origin). */
export async function narrateIrradiance(region, { voice_id = 'leo', signal } = {}) {
  const url = liveUrl('/api/irradiance/narrate')
  if (!url) throw new Error('Narration needs the live API (run `python -m server`, or set VITE_API_BASE)')
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ region, voice_id }),
    signal,
  })
  if (!res.ok) {
...
/** Script text for on-screen transcript. */
export async function loadIrradianceScript(region, { signal } = {}) {
  const url = liveUrl(`/api/irradiance/script/${encodeURIComponent(region)}`)
  if (!url) throw new Error('Script needs the live API (run `python -m server`, or set VITE_API_BASE)')
