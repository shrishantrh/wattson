...
const ABSENT = { no_documents_ingested: 'grid-measured', no_site_resolved: 'utility not yet named' }
    ...
    column = (
      <>
        ...
        <Card title={<><b>Companies</b> · talk vs walk{data.is_mock && <> · <span className="accent">mock</span></>}</>} right={<CopyButton text={() => window.location.href} label="Copy link" />} onClose={back}>
          ...
          <div className="nums"><Num value={String(list.length)} label="operators in the data" sub={`${withClaims.length} with their own filings read, ${noDocs.length + unmapped.length} measured straight off the grid`} /><Num value={String(n)} label="claims pulled from filings" sub={`${n - cv} settled against the meter`} /><Num value={String(pts.length)} label="sites located" sub={'each one found from the utility that serves it, never guessed from the state'} /></div>
        ...
        <Card title={<>What each company <b>claims</b>, against what its grids <b>generate</b></>}>
          ...
          {lines.length > 0 && (
            <ul className="co-lines">
              {lines.map(c => (
                <li key={keyOf(c)}><a href={href.check(keyOf(c))}><b>{c.ticker || c.id}</b></a> {plural(c.n_claims, 'claim')} read across {plural(c.n_sites, 'site')} · grid data for {pct0(c.coverage)} of those sites</li>
              ))}
            </ul>
          )}
