    ...
    <>
      ...
      <Card title={<b>What we found</b>} onClose={back}>
        ...
        {scene === 'detector' && <div className="fd-scene">
          ...
          <p className="note pg-fine">{(() => { const by = traj.summary?.by_year?.[yi]; const d = traj.summary?.biggest_drop?.[0], u = traj.summary?.biggest_rise?.[0]; const ex = traj.summary?.excluded_step_changes || []; const natY = nat[String(yp.year)]?.overnight; return by && d && u ? `In ${yp.year} the US ran ${natY != null ? pct1(natY) : '—'} clean at night. From 2019 to 2025 the biggest fall among grids was ${coords.regions[d.id]?.label || d.id} (${Math.round(d.from * 100)}% to ${Math.round(d.to * 100)}%), the biggest rise ${coords.regions[u.id]?.label || u.id} (${Math.round(u.from * 100)}% to ${Math.round(u.to * 100)}%) — where a datacenter lands decides what burns for it. Zones are coloured with their grid's share, since zones report demand only.${ex.length ? ` ${ex.map(id => coords.regions[id]?.label || id).join(', ')} show a single-year step in the published data and are left out.` : ''} Press play.` : '' })()}</p>
        </div>}
      </Card>
