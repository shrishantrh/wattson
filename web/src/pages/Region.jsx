...
import { regionTitle, pts1, n0, caveatFor, correctedBaseline } from '../lib/findings.js'
...
export default function Region({ route }) {
  ...
  const corr = correctedBaseline(data)
  const change = corr != null && share25 != null ? share25 - corr : sit.change_since_2019
  ...
  const column = (
    <>
      <Card title={<><b>{c?.label || data.name}</b> · {c?.place || data.ba_name}</>} right={<CopyButton text={() => window.location.href} label="Copy link" />} onClose={back}>
        ...
        <div className="nums"><Num num={share25 != null ? share25 * 100 : undefined} value="—" format={pctFmt} label="clean power at night, 2025" sub={`${pts1(change)} since 2019${corr != null ? ', corrected' : ''}`} accent /><Num value={det.rank ? `#${det.rank}` : '—'} label={`of ${det.n_scored || 111} for new flat load`} sub={det.pattern} /><Num num={data.demand?.['2025']?.overnight_avg_mw} format={n => n0(n)} value="—" label="MW demand at night, 2025" sub={data.demand?.['2019']?.overnight_avg_mw ? `${n0(data.demand['2019'].overnight_avg_mw)} in 2019` : null} /></div>
        {cav && <div className="banner banner-error" style={{ marginTop: 12 }}>{cav}</div>}
        <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}>{data.cf_inherited_from_ba && <Chip small>generation figures are the whole {data.ba} grid</Chip>}<Chip small href={href.compare({ mw: 300, metros: [id] })}>Compare 300 MW here</Chip></div>
      </Card>
