  ...
  const column = (
    <>
      <Card title={<><b>{c?.label || data.name}</b> · {c?.place || data.ba_name}</>} right={<CopyButton text={() => window.location.href} label="Copy link" />} onClose={back}>
        ...
        <div className="nums"><Num num={share25 != null ? share25 * 100 : undefined} value="—" format={pctFmt} label="clean power at night, 2025" sub={`${pts1(sit.change_since_2019)} since 2019`} accent /><Num value={det.rank ? `#${det.rank}` : '—'} label={`of ${det.n_scored || 111} for new flat load`} sub={det.pattern} /><Num num={data.demand?.['2025']?.overnight_avg_mw} format={n => n0(n)} value="—" label="MW demand at night, 2025" sub={data.demand?.['2019']?.overnight_avg_mw ? `${n0(data.demand['2019'].overnight_avg_mw)} in 2019` : null} /></div>
