  ...
  const column = (
    <>
      ...
      <Card title={<b>Data</b>} onClose={back}>
        ...
        <p className="pg-lede">Hourly EIA-930 via PUDL through 2026-09-05. Claims are read off the rendered pages of each company's own reports; site mapping and operators are hand-curated.</p>
      </Card>
      {loading ? <Card><Loading what="the sheet" /></Card> : error ? <Card><ErrorState error={error} onRetry={reload} /></Card> : <Card><Table key={t} columns={data.columns} rows={data.rows} sortKey={data.sortKey} defaultSort={data.defaultSort} rowHref={data.rowHref} rowKey={data.rowKey} filter filterPlaceholder={`Filter ${data.rows.length} rows`} csvName={`wattson-${t}`} dense maxHeight="min(66vh, 720px)" /></Card>}
    </>
  )
