  ...
  if (loading) column = <>{crumb}<Card title={<b>What changed</b>} onClose={back}><Loading what="alerts" /></Card></>
    ...
    <>
      ...
      <Card title={<><b>What changed this month</b> · trailing 12 months to {month(data.latest_month)}</>} right={<CopyButton text={() => window.location.href} label="Copy link" />} onClose={back}>
        ...
        <p className="note pg-fine">{sentence} Severity = how far past the threshold × how many months it has held × how recent; the bar is that severity, and it runs ember while an alert is in the primary tier. {data.excluded_regions?.length ? `Excluded: ${data.excluded_regions.join(', ')} (data flag).` : ''}</p>
      </Card>
      <Card>
